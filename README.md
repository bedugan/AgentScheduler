...
The installed extension fingerprints its packaged worker and deploys an immutable copy under VS Code global storage at `worker/<fingerprint>/workerCli.js`; the Wakeup Trigger points there so extension-version cleanup cannot invalidate an existing trigger. The Local Store remains `agent-scheduler.sqlite` in global storage. On macOS, the generated plist is stored in `~/Library/LaunchAgents` as required by launchd. The Wakeup Trigger requires a standalone absolute Node.js executable that can load `node:sqlite` and execute a Worker CLI dry run; VS Code's Electron executable is not used. If Node is not discoverable on `PATH`, set `AGENT_SCHEDULER_NODE_PATH` to the absolute `node` or `node.exe` path before starting VS Code. When the platform or runtime is unsupported, scheduling/editing and Manual Run Now remain available while Local Scheduling actions show prerequisite guidance.
